...
function connect_db(){
	...
	mysql_select_db("elgg3") or die(mysql_error());
...
//ADD a new object_entity,
function create_objects_entity($id,$count){	
...
//HABRÏA que añadir una funcion que controle si el lenguage que se quiere crear está creado, se puede hacer siguiendo la nomenclatura de las siglas
// y mirarlo con un mysql_fetch_assoc.
function add_language($id){
	begin_profiling();
	$query = "ALTER table elgg_objects_entity add column description_$id text";
	$result = mysql_query($query);
	analyse();
}
...
function write_to_file($FileHandler, $data){
	...
	// 	- Esta acción sería como esta implementada, el lenguage sería el por defecto.
	// - Traducir blog
	// 	- Vamos a crear multiples lenguages y vamos a ir metiendo ahí pues como 100 o así.
	// - Acceder al blog
	// 	- Se puede hacer un test también de lectura.
		...
		create_objects_entity($id,$count);	
